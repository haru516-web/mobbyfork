...
$TokaDir = Join-Path $Root 'docs/miyake-yuko/img/8mobby'
...
function DrawTop3($g,$spec,$map){ Header $g $spec.Title $spec.Sub $spec.Accent; for($i=0;$i -lt 3;$i++){ $x=72+$i*312; RR $g $x 170 286 310 30 (B '#fffdfb') (Pn $spec.Colors[$i] 4); T $g (($i+1).ToString()+'位') $RankF (B $spec.Colors[$i]) ($x+18) 182 80 45 'Near'; I $g $map[$spec.Order[$i]] ($x+38) 230 210 145; T $g $spec.Order[$i] $Mini (B $spec.Colors[$i]) ($x+18) 382 250 62 }; for($i=3;$i -lt 8;$i++){ Row $g ($i+1) $spec.Order[$i] $map[$spec.Order[$i]] 90 (510+($i-3)*158) 900 130 $spec.Colors[$i%$spec.Colors.Count] '#fffdfb' } }
...
for($idx=0;$idx -lt $Specs.Count;$idx++){
  ...
  if(Test-Path -LiteralPath $bgPath){ $bg=[Drawing.Image]::FromFile($bgPath); $g.DrawImage($bg,0,0,1080,1500); $bg.Dispose(); $veil=New-Object Drawing.SolidBrush ([Drawing.Color]::FromArgb(92,255,255,255)); $g.FillRectangle($veil,0,0,1080,1500); $veil.Dispose() } else { $g.Clear([Drawing.ColorTranslator]::FromHtml('#fff5e8')) }
  switch($s.Layout){ 'top3' { DrawTop3 $g $s $Map } 'split' { DrawSplit $g $s $Map } default { DrawSimple $g $s $Map } }
  $file=Join-Path $OutDir ('simple-korean-ranking-{0:D2}-agent-ai-8mobby.png' -f ($idx+1))
