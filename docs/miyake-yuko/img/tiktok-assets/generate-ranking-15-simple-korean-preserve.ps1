...
$BgDir = Join-Path $OutDir 'ai-backgrounds'
...
for($idx=0;$idx -lt $Specs.Count;$idx++){
  ...
  $bgPath=Join-Path $BgDir ('simple-korean-ranking-{0:D2}-bg.png' -f ($idx+1))
  ...
  $file=Join-Path $OutDir ('simple-korean-ranking-{0:D2}-ai-preserve.png' -f ($idx+1))
