...
$MobbyDir = Join-Path $Root 'docs/miyake-yuko/img/8mobby'
$BgDir = Join-Path $OutDir 'agent-ai-backgrounds'
...
function Header($g,$title,$sub,$accent){
  T $g $title $Title (B $accent) 44 28 992 66
  T $g $sub $Sub (B '#3a2a2a') 100 96 880 34
  RR $g 170 1416 740 44 22 (B '#fffdfb') (Pn '#f0a5b9' 2)
  T $g 'ビューティムード診断' $Foot (B $accent) 180 1420 720 35
}
function SourceCard($g,$rank,$name,$path,$x,$y,$w,$h,$accent){
  RR $g $x $y $w $h 22 (B '#fffdfb') (Pn $accent 3)
  RR $g ($x+12) ($y+12) 50 50 16 (B $accent) $null
  T $g ([string]$rank) $RankF (B '#ffffff') ($x+12) ($y+12) 50 50
  $imgSize=[Math]::Min($h-28, $w*0.38)
  ImgFit $g $path ($x+78) ($y+14) $imgSize $imgSize
  T $g $name $Label (B $accent) ($x+92+$imgSize) ($y+20) ($w-$imgSize-110) 78 'Near'
}
function BigCard($g,$rank,$name,$path,$x,$y,$w,$h,$accent){
  RR $g $x $y $w $h 26 (B '#fffdfb') (Pn $accent 4)
  RR $g ($x+16) ($y+14) 58 48 16 (B $accent) $null
  T $g (($rank).ToString()+'位') $Small (B '#ffffff') ($x+16) ($y+14) 58 48
  ImgFit $g $path ($x+24) ($y+70) ($w-48) ($h-132)
  T $g $name $Mini (B $accent) ($x+16) ($y+$h-56) ($w-32) 40
}
function DrawSimple($g,$spec,$map){ Header $g $spec.Title $spec.Sub $spec.Accent; for($i=0;$i -lt 8;$i++){ SourceCard $g ($i+1) $spec.Order[$i] $map[$spec.Order[$i]] 64 (168+$i*151) 952 128 $spec.Colors[$i%$spec.Colors.Count] } }
function DrawTop3($g,$spec,$map){ Header $g $spec.Title $spec.Sub $spec.Accent; for($i=0;$i -lt 3;$i++){ BigCard $g ($i+1) $spec.Order[$i] $map[$spec.Order[$i]] (58+$i*330) 160 306 355 $spec.Colors[$i] }; for($i=3;$i -lt 8;$i++){ SourceCard $g ($i+1) $spec.Order[$i] $map[$spec.Order[$i]] 86 (548+($i-3)*165) 908 140 $spec.Colors[$i%$spec.Colors.Count] } }
function DrawSplit($g,$spec,$map){ Header $g $spec.Title $spec.Sub $spec.Accent; for($i=0;$i -lt 8;$i++){ $col=$i%2; $row=[math]::Floor($i/2); BigCard $g ($i+1) $spec.Order[$i] $map[$spec.Order[$i]] (60+$col*510) (168+$row*302) 450 270 $spec.Colors[$i%$spec.Colors.Count] } }

$Title=F 'HGP創英角ﾎﾟｯﾌﾟ体' 45 'Bold'; $Sub=F 'HG丸ｺﾞｼｯｸM-PRO' 22 'Bold'; $Label=F 'HG丸ｺﾞｼｯｸM-PRO' 25 'Bold'; $Small=F 'HG丸ｺﾞｼｯｸM-PRO' 17 'Bold'; $Mini=F 'HG丸ｺﾞｼｯｸM-PRO' 20 'Bold'; $RankF=F 'Comic Sans MS' 30 'Bold'; $Foot=F 'HG丸ｺﾞｼｯｸM-PRO' 18 'Bold'
$Map=@{}; Get-ChildItem -LiteralPath $MobbyDir -Filter '*.png' | ForEach-Object { $Map[$_.BaseName]=$_.FullName }
...
for($idx=0;$idx -lt $Specs.Count;$idx++){
  ...
  $bgPath=Join-Path $BgDir ('simple-korean-ranking-{0:D2}-agent-bg.png' -f ($idx+1))
  if(Test-Path -LiteralPath $bgPath){ $bg=[Drawing.Image]::FromFile($bgPath); $g.DrawImage($bg,0,0,1080,1500); $bg.Dispose(); $veil=New-Object Drawing.SolidBrush ([Drawing.Color]::FromArgb(130,255,255,255)); $g.FillRectangle($veil,0,0,1080,1500); $veil.Dispose() } else { $g.Clear([Drawing.ColorTranslator]::FromHtml('#fff5e8')) }
  switch($s.Layout){ 'top3' { DrawTop3 $g $s $Map } 'split' { DrawSplit $g $s $Map } default { DrawSimple $g $s $Map } }
  $file=Join-Path $OutDir ('simple-korean-ranking-{0:D2}-agent-ai-8mobby-source-lock.png' -f ($idx+1))
